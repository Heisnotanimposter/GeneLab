...
!pip install -q biopython stable-baselines3 gymnasium shimmy pyspark streamlit plotly
...
# DNA Parser Class
class DNAParser:
    def __init__(self, encoding='one-hot'):
        """
        Initialize the DNAParser with a specified encoding.

        Parameters:
        encoding (str): The type of encoding to use ('one-hot' or 'integer').
        """
        ...
        if self.encoding == 'one-hot':
            ...
            raise ValueError("Unsupported encoding type. Choose 'one-hot' or 'integer'.")
...
# Step 6: Create a Streamlit Web Application for DNA Data Upload and 3D Visualization

# Save the following code in a separate file named `app.py` and run it using the command:
...
def main():
    st.title("GeneLab DNA Data Visualization")

    st.sidebar.header("Upload DNA Data")
    uploaded_file = st.sidebar.file_uploader("Choose a TXT file containing DNA sequences", type="txt")

    encoding_option = st.sidebar.selectbox("Choose Encoding Type", ["one-hot", "integer"])

    if uploaded_file is not None:
        # Read the file
        stringio = StringIO(uploaded_file.getvalue().decode("utf-8"))
        dna_sequences = stringio.read().strip().split('\n')
        dna_sequences = [seq.strip().upper() for seq in dna_sequences if seq.strip()]
        
        st.header("Uploaded DNA Sequences")
        st.write(dna_sequences)

        # Encode the sequences
        parser = DNAParser(encoding=encoding_option)
        encoded_sequences = parser.encode_sequences(dna_sequences)
        st.header("Encoded DNA Sequences")
        st.write(encoded_sequences)

        # Perform PCA to reduce to 3 dimensions for visualization
        if encoded_sequences.shape[1] >= 3:
            ...
            fig = px.scatter_3d(pca_df, x='PC1', y='PC2', z='PC3',
                                title='3D PCA of Encoded DNA Sequences',
                                labels={'PC1': 'Principal Component 1',
                                        'PC2': 'Principal Component 2',
                                        'PC3': 'Principal Component 3'})
            st.plotly_chart(fig)
        else:
            st.warning("Encoded sequences do not have enough features for 3D PCA visualization.")

    else:
        st.info("Please upload a TXT file containing DNA sequences.")
